...
`project_verification_workbench/stage2_quality_results.json`.
...
Create `project_verification_workbench/stage2_quality_plan.json` and its human
summary `project_verification_workbench/stage2_quality_plan.md`. For each
...
the confirmed Stage 1 Profile handoff with `validate_gate.py profile`. It
...
Stage 1 Profile before any dispatch. It then requires a current, approved
