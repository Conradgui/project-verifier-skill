#!/usr/bin/env python3
"""Evaluate receipt-bound Benchmark comparisons without universal scores."""
...
CONTRACT_PATH = Path(__file__).resolve().parents[1] / "scripts" / "validate_benchmark_task.py"
SPEC = importlib.util.spec_from_file_location("project_verifier_benchmark_contract", CONTRACT_PATH)
CONTRACT = importlib.util.module_from_spec(SPEC)
assert SPEC.loader is not None
SPEC.loader.exec_module(CONTRACT)
GATE_PATH = Path(__file__).resolve().parents[1] / "scripts" / "validate_gate.py"
GATE_SPEC = importlib.util.spec_from_file_location("project_verifier_gate", GATE_PATH)
