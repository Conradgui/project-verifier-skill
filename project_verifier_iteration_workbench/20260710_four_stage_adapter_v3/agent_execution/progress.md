...
- Task 8 已完成：完整离线套件发现 108 项且全部通过，Shell、编译、JSON、Skill validator、安装 dry-run 与合同残留检查通过。收尾后移除了临时历史迁移矩阵及其 3 个专属测试，避免以 50 条 pending 映射制造持续维护负担；现行合同仍不主张完整历史测试等价。
