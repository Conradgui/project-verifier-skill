...
历史迁移矩阵共 69 行：19 行标为 `ported`，50 行仍为 `pending`，没有 `retired_contract` 行。因此，当前测试证明的是**现行四阶段合同**，不是对全部历史测试逐项等价迁移的证明。为避免把私用 Skill 变成无收益的历史追溯工程，本轮不补写这 50 行映射，也不作“完整历史等价”的主张。
